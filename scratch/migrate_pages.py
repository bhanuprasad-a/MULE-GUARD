...
root_dir = "frontend"
bank_dir = "frontend/bank"

root_files = [f for f in os.listdir(root_dir) if f.endswith(".html") and f not in ["login.html", "three-demo.html", "access-denied.html"]]
...
# 1. Update root files with auth guard and dynamic sign out
for filename in root_files:
    ...
    with open(filepath, "w", encoding="utf-8") as f:
        f.write(content)
    print(f"Updated root page: {filename}")

# 2. Update bank files with auth guard, styles paths, logo paths, sign out, and custom sidebars
for filename in bank_files:
    ...
    with open(filepath, "w", encoding="utf-8") as f:
        f.write(content)
    print(f"Updated bank page: {filename}")
